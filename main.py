...
if __name__ == "__main__":
    ...
    while True:
        try:
            site = int(input('1. Unsplash\n2. Pexels\nEnter your choice: '))
        except:
            print('Please enter a number. Try again later')
        if site != sp.UNSPLASH and site != sp.PEXELS:
            print('Please enter correct number. Try again later')
            continue
        else:
            break
    ...
    if site == sp.UNSPLASH:
        folder_prefix = "./unsplash/"
    elif site == sp.PEXELS:
        folder_prefix = "./pexels/"

    result_folder = folder_prefix + query + \
        "/" if len(query) > 1 else "./images/"
    img_url = sp.extract_userinput(query, site)
    prefs = {"download.default_directory": os.getcwd() + result_folder}
    opts.add_experimental_option("prefs", prefs)
    browser = Chrome(options=opts)
    sp.extract_and_save_imgs(
        site, browser, img_url, scroll, result_folder)
    ...
    #         browser = Chrome(options=opts)
    #         sp.extract_and_save_imgs(
    #             site, browser, href, scroll, result_folder)
